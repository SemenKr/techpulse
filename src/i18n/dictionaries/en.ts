export const en = {
  common: {
    language: "Language",
    notifications: "Notifications",
    openNavigation: "Open navigation",
    closeNavigation: "Close navigation",
    searchPlaceholder: "Search signals, sources, or topics",
    switchToSystem: "Use system theme",
    switchToDark: "Switch to dark theme",
    switchToLight: "Switch to light theme",
  },
  sidebar: {
    subtitle: "Intelligence hub",
    nav: {
      overview: "Overview",
      live: "Live Feed",
      signals: "Signals",
      analytics: "Analytics",
      alerts: "Alerts",
      search: "Search",
      settings: "Settings",
    },
  },
  dashboard: {
    eyebrow: "Real-time overview",
    title: "Tech intelligence dashboard",
    description:
      "Monitor technology signals, live updates, and emerging trends from one operational workspace.",
    liveStatus: "Live monitoring ready",
    metricsHeading: "Key metrics",
    metrics: {
      trackedSignals: {
        title: "Tracked Signals",
        description: "Across product, AI, and security feeds",
        trend: "+12.4%",
        trendLabel: "vs last week",
      },
      liveSources: {
        title: "Live Sources",
        description: "Currently monitored channels",
        trend: "+6",
        trendLabel: "new sources",
      },
      marketMomentum: {
        title: "Market Momentum",
        description: "Topic acceleration across watched categories",
        trend: "+3.2%",
        trendLabel: "24h change",
      },
      verifiedAlerts: {
        title: "Verified Alerts",
        description: "High-confidence intelligence items",
        trend: "stable",
        trendLabel: "quality bar",
      },
    },
    analytics: {
      title: "Analytics",
      subtitle:
        "Signal movement and topic concentration across watched technology categories.",
      action: "View report",
      activityTitle: "Streaming intelligence activity",
      activitySubtitle: "Signal volume",
      range: "Last 12 hours",
      trendingTitle: "Trending topics",
      trendingSubtitle: "Current focus areas",
    },
    activity: {
      title: "Realtime activity",
      subtitle: "Recent priority signals from monitored sources.",
      feedTitle: "Priority feed",
      feedSubtitle: "Verified intelligence",
      categoryMixTitle: "Category mix",
      categoryMixSubtitle: "Placeholder breakdown",
    },
    topics: [
      "AI infrastructure",
      "Developer platforms",
      "Security automation",
      "Edge inference",
      "Semiconductor supply",
      "Open model tooling",
    ],
    feed: [
      {
        title: "AI infrastructure demand continues to accelerate",
        timestamp: "2m ago",
        category: "AI",
        description:
          "New capacity signals detected across cloud and chip vendors.",
      },
      {
        title: "Security tooling receives elevated enterprise attention",
        timestamp: "11m ago",
        category: "Security",
        description: "Multiple vendor updates point to consolidation pressure.",
      },
      {
        title: "Developer platform consolidation trend detected",
        timestamp: "24m ago",
        category: "DevTools",
        description:
          "Repository activity and release cadence are moving together.",
      },
      {
        title: "Semiconductor outlook shows stronger momentum",
        timestamp: "41m ago",
        category: "Hardware",
        description: "Supply-side indicators improved across monitored sources.",
      },
    ],
    categories: {
      ai: "AI",
      security: "Security",
      devtools: "DevTools",
      hardware: "Hardware",
    },
  },
}
